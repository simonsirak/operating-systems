...
int main(int argc, char *argv[]) {
  ...
  while((entry = readdir(dirp)) != NULL) {
    ...
    /* we need to read the file stat info relative to the directory we're in */
    struct stat file_st;
    ...
    printf("\tsize: %8lu",  file_st.st_size);
    printf("\tinode %8lu",  file_st.st_ino);
    printf("\trdev %8lu",  file_st.st_rdev);    
    printf("\tname: %s\n", entry->d_name);      
